...
exports.getSubtiposFilter = async (req, res) => {
    try {
        req.db.query('CALL ObtenerSubtiposHijosRecursosPedagogicos()',
            (error, results) => {
                if (error) {
                    console.error('Error en la consulta:', error);
                    return res.status(500).json({ error: 'Error en la base de datos', detalle: error.message });
        ...
        console.error('Error en getSubtiposFilter:', error);
        res.status(500).json({
            error: 'Error al obtener getSubtiposFilter',
            detalle: error.message
        });
    }
};